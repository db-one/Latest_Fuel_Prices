...
SCAN_INTERVAL = datetime.timedelta(hours=1) # 更新间隔时间
ICON = 'mdi:gas-station'

PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    vol.必要的(CONF_NAME): cv.string，
    vol.必要的(CONF_REGION): cv.string，
})
...
class OilPriceSensor(Entity):
    ...
    def update(self):
        ...
        self._entries["update_time"] = datetime.datetime。当前()。strftime('%Y-%m-%d')
